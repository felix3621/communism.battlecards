<style lang="less" global>
  ...
  #Background {
    ...
    {/if}
<script>
    import { TurnamentGame, Card, Errors } from "$lib";
    ...
    function CreateSocketConnection() {
        if (socket)
            socket.close(1000);
        socket = new WebSocket(`ws://${window.location.host}/socket/game?tournamentPanel=true`);
    ...
    function isEqual(obj1, obj2) {
        // If both are not objects or null, check for strict equality
